...
class _ChatCodePageState extends State<ChatCodePage> {
  ...
  Widget build(BuildContext context) {
    // TODO: Sayfa tasarımını düzelt. Yanlış kod girilirseyi düzelt.
                    ...
                    onPressed: () {
                      if (code.text.isNotEmpty) {
                        if (code.text == "Password@123") {
                          ...
                          List<Map<String, dynamic>> listOfMealsInMealList =
                              new List();
                          Map<String, dynamic> userMap = new Map();
                          Map<String, dynamic> mealListMap = new Map();
                          showAlertDialog(context);
                          databaseReference
                              .where('chatCode', isEqualTo: code.text)
                              .get()
                              .then((value) {
                            if (value.docs.isNotEmpty) {
                              if (value.docs.length != 1) {
                                Navigator.pop(
                                    context); // TODO ADMIN HATA YAPTIĞI DURUM. AYNI CHAT CODE 1 KİŞİDEN FAZLASINA VERİLMİŞ
                              }
                              ...
                              FirebaseFirestore.instance
                                  ...
                                  .where('mealListCode',
                                      isEqualTo: userMap['mealListCode'])
                                  .get()
                                  .then((value) {
                                value.docs.forEach((element) {
                                  mealListMap.addAll(element.data());
                                  List list =
                                      []; // Bu list tüm yiyecek kodlarını barındırır. (1 yiyecek için olan mealCode)
                                  mealListMap['mealNumbers']
                                      .forEach((key, value) {
                                  ...
                                  FirebaseFirestore.instance
                                    ...
                                    Navigator.pushReplacement(context,
                                        MaterialPageRoute(
                                            builder: (BuildContext context) {
                                      return tabPages(userMap, mealListMap,
                                          listOfMealsInMealList);
                                    }));
                                  });
                                });
                              });
